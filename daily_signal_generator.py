...
# --- [1. 전략 파라미터 설정] ---
ASSET_NAMES = ['한국 주식', '중국 주식', '일본 주식', '인도 주식', '채권 30년', '채권 10년', 'TIME (나스닥 주식)', 'TIME (나스닥 채권)']
TICKER_MAP = {
...
# --- [2. 텔레그램 전송 함수 (마크다운/HTML 제거)] ---
def send_telegram_message(token, chat_id, message):
    url = f"https://api.telegram.org/bot{token}/sendMessage"
    # parse_mode 파라미터를 제거하여 순수 텍스트로 전송 (400 에러 원천 차단)
...
def get_daily_signals_and_report():
    ...
    for n in ASSET_NAMES:
        chg_sign = "+" if changes[n] >= 0 else ""
        report.append(f"- {n}: {prices[n]:,.0f} ({chg_sign}{changes[n]:.1%})")
    ...
    for n in ASSET_NAMES:
        ...
        for w in MA_WINDOWS:
            ...
            s_text = "ON" if today_ma_states_dict[k] else "OFF"
            report.append(f"  - {w}일: {s_text} ({disp:+.1%})")
